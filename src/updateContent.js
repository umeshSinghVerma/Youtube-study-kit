...
async function saveImage(videoId, imgUrl, imgText, ytLink, timestamp, videoHeading) {
    ...
    if (previousData) {
        ...
        if (previousVideoData) {
            const sortedData = [...previousData[videoId].data, { imgUrl, imgText, ytLink, timestamp }].sort(
...
function addCaptureButton() {
    let captureButton = document.getElementById("screenshot-button");
    if (captureButton) {
